...
BUILD_TAG="${CUDA_VER}-${CUDA_TYPE}-${LINUX_VER}"
...
if [ -z "$PYTHON_VER" ] ; then
  ...
  gpuci_logger "PYTHON_VER is set to '$PYTHON_VER', adding to build args..."
  BUILD_ARGS="${BUILD_ARGS} --build-arg PYTHON_VER=$PYTON_VER"
fi
